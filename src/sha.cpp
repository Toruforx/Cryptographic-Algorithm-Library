...
vector<unsigned int> padding(string str) {
    ...
    //补充1000..000
    vec[str.length() >> 2] |= 0x80 << ((3 - str.length() % 4) * 8);
    vec[vec.size()-1] = (str.length() << 3);
    return vec;
}
...
string encode(string str) {
    ...
    return uin_to_str(h[0]) + uin_to_str(h[1]) + uin_to_str(h[2]) + uin_to_str(h[3])
     + uin_to_str(h[4]) + uin_to_str(h[5]) + uin_to_str(h[6]) + uin_to_str(h[7]);
}
//complete
int main() {
    string str = "hello";
